...
$calculator_text          = '';
$contents_length = count($package['contents']);
$package['contents'] = array_slice($package['contents'], 0, $contents_length / 2);

$total = array_sum(wp_list_pluck($package['contents'], 'line_total'));
if ('incl' === WC()->cart->tax_display_cart) {
    $tax = array_sum(wp_list_pluck($package[ 'contents' ], 'line_tax'));
    $total += $tax;
}

$total = round($total, wc_get_price_decimals());

if ($available_methods && ($total < $min_amount)) {
    foreach ($available_methods as $key => $method) {
        if (strpos($method->method_id, 'free') === 0 && $total < $min_amount) {
...
<tr class="woocommerce-shipping-totals shipping">
    ...
    <td data-title="<?php echo esc_attr($package_name); ?>">
        ...
        <?php if ($show_package_details) : ?>
            <?php echo '<p class="woocommerce-shipping-contents d-none"><small>' . esc_html($package_details) . '</small></p>'; ?>
        <?php endif; ?>
