...
function horizontal_product_card($product_id, $return = true)
{
    ...
    $name = sprintf(
        '<h4 class="%2$s"><a href="%3$s">%1$s</a></h4>',
    ...
    $figure = sprintf(
        '<figure class="%1$s"><a href="%5$s"><img src="%2$s" alt="%3$s" class="%4$s" /></a></figure>',
    ...
    $section = sprintf(
        '<section class="%4$s">%1$s %2$s %3$s</section>',
        $name,
        $short_description,
        $attributes,
    ...
    $article = sprintf(
        '<article data-article="%3$s" class="%2$s" data-popularity="%4$d" data-price="%5$s" data-product-id="%6$d">%1$s</article>',
        $container,
        'px-3 py-1',
        $product->get_name(),
        $sales,
...
function product_add_to_cart($product_id, $return = true)
{
    ...
    $button_minus = sprintf(
        '<button type="%s" class="%s" data-target="%d">%s</button>',
        'button',
        'btn bg-success-dark-20 js-make-me-less border-success-dark-30 text-white border-right-0 btn-plusle-minum',
        $product_id,
        '-'
    );
    $button_plus = sprintf(
        '<button type="%s" class="%s" data-target="%d">%s</button>',
        'button',
        'btn bg-success-dark-20 js-make-me-more border-success-dark-30 text-white border-left-0 btn-plusle-minum',
        $product_id,
        '+'
    );
    $input_amount = sprintf(
        '<input type="%7$s" step="%6$d" min="%5$d" value="%4$d" size="%3$d" pattern="[0-9]*" inputmode="numeric" name="%1$s" id="%1$s" class="%2$s">',
        'quantity',
        'text-center border-success-dark-30',
        4,
        ($steps > 0) ? $steps : 1,
        ($steps > 0) ? $steps : 0,
        ($steps > 0) ? $steps : 1,
    ...
    $input_hidden = sprintf(
        '<input type="%1$s" %1$s class="%2$s" aria-hidden="true" value="%3$d" name="%4$s" id="%4$s">',
        'hidden',
        'd-none',
        $product_id,
        'product_id'
    ...
    $submit_button = sprintf(
        '<input type="%1$s" class="%2$s" value="%3$s" />',
        'submit',
        'btn bg-success text-uppercase text-white btn-lg btn-order border-0 mt-1',
        'Bestel'
    );
    
    $html = sprintf(
        '<form data-action="%1$s" class="%2$s">%3$s %4$s %5$s %6$s %7$s</form>',
        'product_add_to_cart',
